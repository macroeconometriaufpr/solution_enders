...
%teste de sincronização web-desktop
\begin{enumerate}
	\item	
	\begin{enumerate}
		\item 
...
\item %7
 ...
 \end{enumerate}
	...
	\begin{enumerate}
		...
		\begin{figure}[h]
			\centering
			\includegraphics{C:/Users/Lucca/GoogleDrive/DOUTORADO_UFPR/Macroeconometria/solution_enders/10d.eps}
		\end{figure}
		\
	\end{enumerate}
	...
	------------------------------------
		...
		\begin{enumerate}
			\item %12a
				...
				\item %12aii
				\begin{align*}
					y_t&=3+0.25y_{t-1}+0.375y_{t-2}+\varepsilon_t\\
					\\
					\text{parte determinística:}&\\
					y_t&=b_0\\
					\Rightarrow b_0&=3+0.25b_0+0.375b_0\\
					b_0&=\frac{3}{(0.375)}=8\\
					\\
					\text{parte estocástica:}&\\
					\sum\limits_{i=0}^{\infty}\alpha_i\varepsilon_{t-i}&=0.25\Bigg[\sum\limits_{i=0}^{\infty}\alpha_i\varepsilon_{t-1-i}\Bigg]+0.375\Bigg[\sum\limits_{i=0}^{\infty}\alpha_i\varepsilon_{t-2-i}\Bigg]+\varepsilon_t\\
					\alpha_0&=1\\
					\alpha_1&=0.25\alpha_0\\
					\alpha_2&=0.25\alpha_1+0.375\alpha_1\\
					\alpha_3&=0.25\alpha_2+0.375\alpha_2\\
					\vdots&\\
					\alpha_i&=0.25\alpha_{i-1}+0.375\alpha_{i-2}\\
					\end{align*}
					
					\begin{align*}
					\Rightarrow \alpha_i&=A_1(-0.5)^i+A_2(0.75)^i\\
					\alpha_0&=1 \Rightarrow A_1=1-A_2\\
					\alpha_i&=(1-A_2)(-0.5)^i+A_2(0.75)^i\\
					\alpha_1&=0.25 \Rightarrow 0.25=(1-A_2)(-0.5)+A_2(0.75)\\
					A_2&=0.6,\;\; A_1=0.4\\
					\\
					\alpha_i&=0.4(-0.5)^i+0.6(0.75)^i\\
					\\
					y_t^p&=8+\sum\limits_{i=0}^{\infty}\Bigg[0.4(-0.5)^i+0.6(0.75)^i\Bigg]\varepsilon_{t-i}
				\end{align*}			
